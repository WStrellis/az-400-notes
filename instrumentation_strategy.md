...
- supports automatic , scheduled availablity tests( pings). Pings can be sent from a variety of geographic locations.
...
*Azure Monitor* - service for collecting, analyzing, and acting on telemetry from cloud and on-prem environments.  
- collects data from Azure platform resources and custom sources via API
- two types of data
    - *Azure Monitor Metrics* - numerical time-series data
    - *Azure Monitor Log Analytics* - resource logs
...
- view IOC for cloud and on-prem systems

*Just In Time (JIT) Access Control* - only allows access to VMs at specific times on certain ports